...
**不同厂商的明细口径完全一致**（千问走 DashScope 协议、MiniMax 走海螺协议，但落到使用记录里是同一套字段）：

| 字段 | 含义 | 千问 | MiniMax |
|---|---|---|---|
| `image_size` | 计费档位 `1K` / `2K` / `4K` | 按上游真实输出尺寸分档 | 按请求尺寸分档（不传时 2K） |
| `image_input_size` | 客户端请求的原始尺寸 | 原样保留 | 原样保留（`16:9` 等宽高比也保留） |
| `image_output_size` | 上游真实输出尺寸 | 填真实值（如 `1664x928`） | 上游不回传，留空 |
| `image_size_source` | 分档依据 `output` / `input` / `default` | `output` | `input` |
| `image_count` | 实际出图张数 | 按实际 | 按实际 |

即：**档位串在明细里长得一样**，差异只在"依据"字段，便于对账。同档位下两个模型的计费算法完全相同（单价 × 张数 × 倍率），单价本身可因模型而异（见分组图片定价卡）。

**与文本调用共同的明细字段**（生图 / 生视频 / 音频与聊天补全一致，不再出现"只有金额、端点与耗时空着"的半截行）：

| 字段 | 含义 |
|---|---|
| `inbound_endpoint` | 客户端请求的入站端点，如 `/v1/images/generations` |
| `upstream_endpoint` | 实际打到的上游端点路径（只保留 path，不含域名） |
| `duration_ms` | 从提交到终态结算的耗时（异步任务含排队与轮询时间） |
| `user_agent` / `ip_address` | 客户端标识与来源 IP |

同步出图（创建即返回）与异步出图（轮询到终态）走的是两条不同的结算路径，但**落库字段与计费档位完全一致**——异步路径会从创建阶段暂存的元数据里回源入站端点与真实输出尺寸，不会退化成空值或错档。
