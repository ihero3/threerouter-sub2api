...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	consumerRepository := repository.ProvideConsumerRepository(client, db)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, consumerRepository, concurrencyService)
	...
	clickCaptchaCache := repository.NewClickCaptchaCache(redisClient)
	clickCaptchaService := service.NewRegistrationClickCaptchaService(clickCaptchaCache)
	authHandler.SetRegistrationClickCaptchaService(clickCaptchaService)
	registrationClickCaptchaHandler := handler.NewRegistrationClickCaptchaHandler(clickCaptchaService)
	...
	adminBlogHandler := admin.NewBlogHandler(blogService)
	...
	policyTemplateService := service.ProvidePolicyTemplateService(compliancePolicyTemplateRepository, complianceAuditLogRepository)
	userJurisdictionMappingRepository := repository.NewUserJurisdictionMappingRepository(db)
	moderationRuleRepository := repository.NewModerationRuleRepository(db)
	moderationRuleService := service.ProvideModerationRuleService(moderationRuleRepository, contentModerationService)
	userComplianceProfileRepository := repository.NewUserComplianceProfileRepository(db)
	userComplianceProfileService := service.NewUserComplianceProfileService(userComplianceProfileRepository)
	complianceMappingService := service.ProvideComplianceMappingService(userJurisdictionMappingRepository, moderationRuleService, userComplianceProfileService)
	complianceCredentialRepository := repository.NewComplianceCredentialRepository(db)
	complianceCredentialService := service.NewComplianceCredentialService(complianceCredentialRepository, complianceService)
	governanceHandler := admin.NewGovernanceHandler(complianceService, policyTemplateService, complianceMappingService, complianceCredentialService)
	moderationRuleHandler := admin.NewModerationRuleHandler(moderationRuleService)
	...
	videoTaskRepo := repository.ProvideVideoTaskRepo(videoTaskRepository)
	accountService := service.NewAccountService(accountRepository, groupRepository)
	videoAdapter := service.ProvideVideoAdapter()
	videoTaskService := service.NewVideoTaskService(videoTaskRepo, accountService, gatewayService, rateLimitService, billingService, apiKeyService, openAIGatewayService, videoAdapter)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, adminBlogHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, governanceHandler, moderationRuleHandler, ticketHandler, auditLogHandler, mediaTaskAdminHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, complianceService, coordinator)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, complianceService, coordinator)
	...
	governanceUserHandler := handler.NewGovernanceUserHandler(complianceService, userComplianceProfileService, policyTemplateService, moderationRuleService, complianceMappingService, complianceCredentialService)
	teamRepository := repository.ProvideTeamRepository(client, db)
	teamMemberRepository := repository.ProvideTeamMemberRepository(client, db)
	userLookupRepository := repository.ProvideUserLookupRepository(userRepository)
	teamService := service.NewTeamService(teamRepository, teamMemberRepository, userLookupRepository, emailService, settingService)
	teamHandler := handler.NewTeamHandler(teamService, userLookupRepository)
	departmentRepository := repository.ProvideDepartmentRepository(client, db)
	...
	teamAnalyticsRepository := repository.ProvideTeamAnalyticsRepository(client, db)
	teamAnalyticsService := service.NewTeamAnalyticsService(teamAnalyticsRepository)
	teamAnalyticsHandler := handler.NewTeamAnalyticsHandler(teamAnalyticsService)
	...
	videoGatewayHandler := handler.NewVideoGatewayHandler(videoTaskService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, registrationClickCaptchaHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, blogHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, handlerTicketHandler, governanceUserHandler, teamHandler, departmentHandler, consumerHandler, teamAnalyticsHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, videoGatewayHandler, mediaGatewayHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
