export default {
    accounts: {
      ...
      cnProviders: {
        ...
        accountType: {
          apiKey: 'API Key',
          apiKeyDesc: '使用官方 API Key 连接，token 长期有效，余额查询可用。',
          oauth: 'Web 会话 OAuth（实验性，仅 Zhipu）',
          oauthDesc: '实验性功能：Zhipu 未提供官方 OAuth 端点，本功能依赖第三方逆向鉴权，可能因 Zhipu 鉴权机制变化而失效。无自动刷新，过期需重新注入。',
          experimentalBanner: '⚠ 实验性功能。Zhipu 官方未提供 OAuth 端点，依赖第三方逆向鉴权。refresh_token 直接当 Bearer 调 chatglm.cn 当前会返回 401。建议优先使用 API Key 模式。',
        },
        accessToken: 'Access Token',
        accessTokenPlaceholder: '填入能通过 chatglm.cn 鉴权的 Bearer 字符串（refresh_token / access_token / Cookie 等）',
        accessTokenHint: '⚠ 实验性功能：Zhipu 官方未提供 OAuth 端点，refresh_token 直接当 Bearer 调 chatglm.cn API 当前会返回 401。需要持续维护逆向方案（cookie 鉴权、签名等）。建议优先使用 API Key 模式（稳定）。Kimi/DeepSeek 的 Web 会话 OAuth 暂不支持。',
        apiProtocol: {
          title: 'API 协议',
          adaptive: '自适应',
      ...
      pleaseEnterAccessToken: '请输入 Access Token（Web 会话 OAuth）',
