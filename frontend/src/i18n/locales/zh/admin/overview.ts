export default {
    ...
    backup: {
      ...
      imageStorage: {
        title: '异步生图对象存储',
        description: '开启后异步生图接口可用，与备份共用同一套 S3 客户端，保存后立即生效，无需重启。凭证齐全时结果转存到对象存储、只把短链接写入 Redis；仅勾选启用但凭证不全时降级为「URL 透传」——直接转发上游的图片链接（有效期由上游决定，客户端需尽快取图），此时不接受 response_format=b64_json。',
        enabled: '启用异步生图',
