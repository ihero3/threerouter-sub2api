export default {
    accounts: {
      ...
      cnProviders: {
        ...
        accountType: {
          apiKey: 'API Key',
          apiKeyDesc: 'Connect via the official API Key. Long-lived token, balance queries supported.',
          oauth: 'Web Session OAuth (Experimental, Zhipu only)',
          oauthDesc: 'Experimental: Zhipu has no official OAuth endpoint. This relies on third-party reverse-engineered auth and may break when Zhipu updates their auth scheme. No auto-refresh; re-inject on expiry.',
          experimentalBanner: '⚠ Experimental. Zhipu provides no official OAuth endpoint; auth relies on third-party reverse engineering. Using refresh_token as Bearer against chatglm.cn currently returns 401. Prefer the API Key mode.',
        },
        accessToken: 'Access Token',
        accessTokenPlaceholder: 'Any Bearer string that authenticates against chatglm.cn (refresh_token, access_token, cookie, etc.)',
        accessTokenHint: '⚠ Experimental: Zhipu provides no official OAuth endpoint. Using refresh_token directly as Bearer against chatglm.cn currently returns 401. Continued maintenance of the reverse-engineered scheme (cookie auth, signing, etc.) is required. Prefer the API Key mode for stability. Kimi/DeepSeek web session OAuth is not supported.',
        apiProtocol: {
          title: 'API Protocol',
          adaptive: 'Adaptive',
      ...
      pleaseEnterAccessToken: 'Please enter the Access Token (Web Session OAuth)',
